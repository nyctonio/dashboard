...
const verifyAdminLogin = async (email,password)=>{
    try {
        ...
        if(await bcrypt.compare(password,admin.password)){
            // creating a token
            token = jwt.sign({id:admin._id,username:admin.email,type:'admin'},JWT_SECRET)
...
app.post('/place-order',async(req,res)=>{
    ...
    try {
        ...
        order.save((err)=>{if(err){throw err;}else{return res.json({status:'ok',data:'order updated sucessfully'})}})
    } catch (error) {
        console.log(JSON.stringify(error));
        return res.json({status:'error',error:JSON.stringify(error)});
    }
...
app.post('/edit-order',async (req,res)=>{
    ...
    if(verifyToken(token)){
        ...
        try {
            ...
            console.log(order);
            res.render('changeorder',{data:Data,username:verify.username,order:order});
        } catch (error) {
            return res.json({status:'error',error:'data took too long to fetch try again'})
        }
    }else{
        res.redirect('/login')
    }
...
app.get('/orders',async (req,res)=>{
    ...
    if(verifyToken(token)){
        ...
        try {
            const verify = jwt.verify(token,JWT_SECRET);
            const Data = await Order.find({user:verify.username}).sort({updatedAt:-1});
            console.log(Data);
...
app.get('/login',(req,res)=>{
    res.render('login',{name:'User Login',aname:'Admin',route:'/login',aroute:'/admin/login'})
})
...
app.get('/admin/login',(req,res)=>{
    res.render('login',{name:'Admin Login',aname:'User',route:'/admin/login',aroute:'/login'})
})
...
app.post('/product/add-update',(req,res)=>{
    const data = req.body;
    Product.find({sno:data.sno},(err,product)=>{
    ...
    if(product.length!=0){
        ...
        product.save(function (err) {
            if(err) {
                ...
                res.json({status:'ok',data:'updated the product'})
            }
        });
    }else{
        try {
